...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SPACER!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  real function EF_elec(n, T, h, ind, dep, lat, v, ft)
    ...
    real               ::NeTot, ipPer, ipTot, ipTex, rad, en_ip
...
!       + v%s4p_elec*(T%s4p - T%elec)&
    ...
    !if( mype .eq. 0 ) then
    !   print *, 'v%sp_elec = ', v%sp_elec
    !   print *, 'v%elec_elecHot = ', v%elec_elecHot
    !   print *, 'Teq = ', Teq
    !endif
...
!    print *, rdist, EF_elec
  ...
  real function EF_ex2(n, T, h, ind, dep, v, ft)
    type(density)      ::n
    type(temp)         ::T
    type(r_ind)        ::ind
    type(r_dep)        ::dep
...
!            +  dep%transport * n%op * ROOTPI * h%op)
    ...
    Teq= v%sp_ex *(T%sp    - T%ex)&
       + v%s2p_ex*(T%s2p   - T%ex)&
       + v%s3p_ex*(T%s3p   - T%ex)&
       + v%op_ex *(T%op  - T%ex)&
       + v%o2p_ex*(T%o2p   - T%ex)&
       + v%ex_elec*(T%elec - T%ex)&
       + v%ex_elecHot*(T%elecHot - T%ex)

  EF_ex2 = Teq + (S - L)/(ROOTPI * h%ex)

  end function EF_ex2
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SPACER!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  real function EF_ex(n, T, h, ind, dep, lat, v, ft)
    type(density)      ::n
    type(temp)         ::T
    type(height)       ::h
    type(r_ind)        ::ind
    ...
    Tex= v%sp_ex *(T%sp  - T%ex)&
       + v%s2p_ex *(T%s2p - T%ex)&
       + v%s3p_ex *(T%s3p - T%ex)&
!       + v%s4p_ex *(T%s4p - T%ex)&
       + v%op_ex *(T%op  - T%ex)&
       + v%o2p_ex *(T%o2p - T%ex)&
       + v%elec_ex*(T%elec - T%ex)

    !if( mype .eq. 0 ) then
    !   print *, 'v%sp_ex = ', v%sp_ex
    !   print *, 'v%elec_ex = ', v%elec_ex
    !   print *, 'Tex = ', Tex
    !endif
...
! ionization potentials
    ...
    ipTex = 2.0 * ipPer * n%ex/3.0

    !if( mype .eq. 0 ) then
    !   print *, '---------Extra iteration------'
    !   print *, 'ex en_ip =', en_ip
    !   print *, 'ex ipPer = ', ipPer
    !   print *, 'ex ipTex = ', ipTex
    !endif

    EF_ex = Tex - (2.0 * rad / 3.0)
  end function EF_ex
